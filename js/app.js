...
console.log("⚡ FIT DATA: App Iniciada (v6.2 - UI Fixes & Avisos)...");
...
// --- INYECCIÓN UI (ESTILOS CORREGIDOS) ---
function injectAppUI() {
    ...
    style.textContent = `
        ...
        /* --- CORRECCIÓN LIBRERÍA (Diseño Compacto) --- */
...
// --- ADMIN: AVISOS + LIBRERÍA OPTIMIZADA ---
window.toggleAdminMode = (mode) => { 
    ...
    if(!annCard) {
        ...
        // CORRECCIÓN: Usar tab-plans como ancla segura para insertar el botón
        const plansTab = document.getElementById('tab-plans');
        if(plansTab && !document.getElementById('tab-announcements')) {
            const btn = document.createElement('button');
            btn.id = 'tab-announcements';
            btn.className = plansTab.className; // Heredar estilo
            btn.innerText = 'AVISOS';
            btn.onclick = () => window.toggleAdminMode('announcements');
            plansTab.parentNode.insertBefore(btn, plansTab.nextSibling);
        }
    }
...
window.loadAdminAnnouncements = async () => {
    ...
    try {
        ...
        snap.forEach(d => {
            const a = d.data();
            const div = document.createElement('div');
            div.className = 'assigned-routine-item'; // Reutilizamos clase base
...
window.createAnnouncement = async () => {
    ...
    try {
        await addDoc(collection(db, "announcements"), {
            ...
            active: false, createdAt: serverTimestamp(), assignedTo: []
        });
...
window.openAssignAnnouncementModal = async (id) => {
    ...
    window.openAssignPlanModal(null);
...
// --- LIBRERÍA OPTIMIZADA (DISEÑO COMPACTO) ---
