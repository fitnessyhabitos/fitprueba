...
console.log("⚡ FIT DATA: App Iniciada (v8.2 - Stability Fixes)...");
...
// Alarma final: Onda Triangular (Punto medio equilibrado)
function playFinalAlarm() {
    ...
    // TRIANGLE: Audaz pero no doloroso
    osc1.type = 'triangle';
    osc2.type = 'triangle';
    ...
    gain.gain.setValueAtTime(1.0, now + 0.6); // Sustain
...
// --- FIX PARA "CANVAS ALREADY IN USE" Y "RANGE ERROR" ---
function renderMuscleRadar(canvasId, stats) {
    const ctx = document.getElementById(canvasId);
    if (!ctx) return;

    // 1. Limpieza robusta: Si ya existe un gráfico en este canvas, destrúyelo.
        ...
        options: {
            scales: {
                r: {
                    ...
                    // Fix para RangeError: Forzar ticks enteros y string
                    ticks: {
                        display: false,
                    ...
                    suggestedMin: 0,
                    // Fix para evitar división por cero si todo es 0
    ...
    // Actualizar referencias globales por si acaso
...
// --- LOGICA DE MOSTRAR AVISOS AL USUARIO ---
async function checkNotices() {
    if(userData.role === 'admin' || userData.role === 'assistant') return;
