...
console.log("⚡ FIT DATA: App Iniciada (Versión Final: Semanal + Live + Coach)...");
...
let communityUnsubscribe = null; // Listener de notificaciones
...
// --- UTILIDADES NUEVAS (Semana ISO & Notifs) ---
...
function initCommunityListener() {
    ...
    // Escuchar último workout añadido
    ...
    communityUnsubscribe = onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach(async (change) => {
            if (change.type === "added") {
                ...
                // Si ocurrió hace menos de 60s y no es mío
...
onAuthStateChanged(auth, async (user) => {
    if(user) {
        ...
        if(snap.exists()){
            ...
            checkPhotoVisualReminder(); // 1. Aviso Foto
            initCommunityListener();    // 2. Notificaciones en vivo
            checkPhotoReminder();       // 3. Aviso horario antiguo
...
window.toggleSuperset = (idx) => {
    if (idx < currentRoutineSelections.length - 1) { currentRoutineSelections[idx].s = !currentRoutineSelections[idx].s; renderSelectedSummary(); }
};
...
window.changeRankFilter = (type, val) => {
    if(type === 'time') { rankFilterTime = val; document.querySelectorAll('#ranking-view .pill').forEach(el => el.classList.remove('active')); document.getElementById(`time-${val}`).classList.add('active'); }
    if(type === 'gender') { rankFilterGender = val; document.querySelectorAll('.pill-selector:nth-child(3) .pill').forEach(el => el.classList.remove('active')); document.getElementById(`gender-${val}`).classList.add('active'); }
...
// --- ADMIN (USERS, LIB, PLANS) CON PUNTO VERDE ---
...
window.loadAdminPlans = async () => {
    ...
    const selector = document.getElementById('plan-routine-selector');
...
// --- AQUÍ ESTÁ LA LÓGICA DEL PUNTO VERDE 🟢 ---
window.loadAdminUsers = async () => {
    ...
    try {
        ...
        // Ordenar: primero los que entrenaron hoy
